...
# 공 크기에 따른 최초 스피드
ball_speed_y = [-18, -15, -12, -9] # index 0, 1, 2, 3에 해당하는 값의 스피드 
...
balls.append({
    ...
    "to_x" : 3, # X축 이동방향, -3 이면 왼쪽으로 3이면 오른쪽으루
    "to_y" : -6, # 6축 이동방향
...
total_time = 100
start_ticks = pygame.time.get_ticks() # 시작 시간 정의하기

# 게임 종료 메시지 / 타임아웃(실패), 미션 성공(성공), 공에 맞음(실패)
game_result = 'Game Over!'

# 이벤트 루프
running = True
while running:
    dt = clock.tick(30) # 화면 초당 프레임 수
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        # 세로 위치
        # 스테이지에 튕겨서 올라가는 처리 하기
        if ball_pos_y >= screen_height - stage_height - ball_height:
            ball_val['to_y'] = ball_val['init_spd_y']
        else: # 그 외의 모든 경우에는 속도를 줄여 증가해야함
            ball_val['to_y'] += 0.5
...
msg = game_font.render(game_result, True, (0, 0, 0)) # 노란색으로 출력
